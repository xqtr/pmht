#!/bin/bash

source ./pref.cfg

function youporn_streamer() {
youtube-dl -q -o - "$1" | mplayer -really-quiet -fs -ao alsa - 
}

#if [ -z $DISPLAY ]
#  then
#    DIALOG=dialog
#  else
#    DIALOG=Xdialog
#  fi

curl -s www.youporn.com > /tmp/yp.$$

cat  /tmp/yp.$$ | grep '<a href="/category/' | sed 's/\(<li>\)/\n\1/g' | sed -n -e 's/.*<a href=\"\(.*\)\">.*/\1/p' > /tmp/cats.$$


if [ "$(cat /tmp/cats.$$)" = "" ]
then 
  dialog --msgbox "No Videos found..." 5 30
  rm -f /tmp/cats.$$
  rm -f /tmp/yp.$$
  exec $menu/restricted.sh
fi

i=1
while read line
do
id=$(echo $line | cut -d"/" -f3)
name=$(echo $line | cut -d"/" -f4)
echo $id "$name" >>/tmp/options.$$ 
  i=`expr $i + 1`
done </tmp/cats.$$
OPTIONS=`cat /tmp/options.$$`

# present menu options

while :
do

dialog --title " Youporn.com " --column-separator , --menu "Select Category:" $l1 $col $l2 ${OPTIONS} 2> answer

if [ "$?" = "0" ]
then
	ch=$(cat /tmp/cats.$$ | grep -w $(cat answer))

	curl -s "www.youporn.com$ch" > /tmp/yp.$$


# Cancel is pressed
else
rm -f /tmp/vids.$$
rm -f /tmp/options.$$
rm -f /tmp/yp.$$
        exec $ecdir/emucom
fi

cat /tmp/yp.$$ | grep '<a href="/watch/' | sed -n -e 's/.*<a href=\"\(.*\)\">.*/\1/p' > /tmp/vids.$$

if [ "$(cat /tmp/vids.$$)" = "" ]
then 
  dialog --msgbox "No Videos found..." 5 30
  rm -f /tmp/vids.$$
  exec $ecdir/emucom
fi

i=1
while read line
do
id=$(echo $line | cut -d"/" -f3)
name=$(echo $line | cut -d"/" -f4)
echo $id "$name" >>/tmp/options1.$$ 
  i=`expr $i + 1`
done </tmp/vids.$$
OPTIONS=`cat /tmp/options1.$$`

# present menu options

dialog --title " Youporn.com " --column-separator , --menu "Select video:" $l1 $col $l2 ${OPTIONS} 2> answer


if [ "$?" = "0" ]
then
	ch=$(cat /tmp/vids.$$ | grep $(cat answer))
 
	youporn_streamer "http://www.youporn.com"$ch

#Change resolution back to normal, if something goes wrong. 
	
	exec $menu/youporn.sh
# Cancel is pressed
else
rm -f /tmp/vids.$$
rm -f /tmp/options.$$
rm -f /tmp/yp.$$
        exec $ecdir/emucom
fi
done
rm -f /tmp/vids.$$
rm -f /tmp/options.$$
rm -f /tmp/yp.$$
exit
